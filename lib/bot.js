...
class Bot {
  constructor(options) {
    ...
    this.nickSuffix = options.userNickSuffix;
    this.nickPrefix = options.userNickPrefix;
  ...
  ircNick(slackName) {
    var ircAssembledNick = slackName;
    if (this.nickSuffix !== undefined) {
      ircAssembledNick = ircAssembledNick.replace(/\./g, '-').substr(0, SERVER_NICKLEN - this.nickSuffix.length)
       + this.nickSuffix;
    }
    if (this.nickPrefix !== undefined) {
       ircAssembledNick = this.nickPrefix + ircAssembledNick.charAt(0).toUpperCase() + ircAssembledNick.slice(1);
    }
    return ircAssembledNick;
  }
